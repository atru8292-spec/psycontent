...
// с OpenAI (по ТЗ). Не путать: это OpenAI-клиент, не OpenRouter. Имя файла и
// экспортов оставлены прежними, чтобы не трогать импорты во всех экранах/роутах,
// переименование не делаем (сломаются импорты).
// Модель ЖЕСТКО одна: DEFAULT_MODEL (gpt-5.4), задается тут. Выбор модели у
// пользователя убран, поле preferred_model в user_settings мертвое, его дефолт
// 'anthropic/claude-sonnet-4-5' вычищен из генераторов (не влиял на вызов).
