...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!profile) return NextResponse.json({ error: 'need_onboarding', message: 'Сначала заполни профиль', redirect: '/onboarding' }, { status: 404 })
...
Block строго: "trend" (12 штук), "science" (10 штук), "quote" (8 штук)`
    ...
    const result = await generateWithWebSearch(prompt, { userId, operation: 'research_topics', knownNames: profile?.full_name ? [profile.full_name] : undefined })
